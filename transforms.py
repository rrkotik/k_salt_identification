...
class RandomContrast:
    def __init__(self, limit=.15, prob=.5):
        self.limit = limit
        self.prob = prob
...
class RandomHueSaturationValue:
    def __init__(self, hue_shift_limit=(-10, 10), sat_shift_limit=(-10, 10), val_shift_limit=(-10, 10), prob=0.5):
...
def augment_airbus_base(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.4, rotate_limit=45, prob=.75),
        OneOf([ImageOnly(RandomBrightness()), ImageOnly(RandomHueSaturationValue()), ImageOnly(RandomContrast())]),
...
def augment_shift_flip_scale11(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.10, rotate_limit=0, prob=.75)
    ])(x, mask)


def augment_shift_flip(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.0, rotate_limit=0, prob=.5)
    ])(x, mask)


def augment_shift_flip_color(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
...
def augment_shift_flip_scale(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.2, rotate_limit=0, prob=.5)
    ])(x, mask)

def augment_shift_flip_rotate(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0., rotate_limit=20, prob=.5)
    ])(x, mask)

def augment_shift(x, mask=None, prob=.5):
    return DualCompose([
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.0, rotate_limit=0, prob=.5)
    ])(x, mask)


def augment_shift_flip_scale1(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        VerticalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.2, rotate_limit=10, prob=.75)
    ])(x, mask)


def augment_shift_flip_scale2(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        VerticalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.3, rotate_limit=30, prob=.75)
    ])(x, mask)
...
def augment_color(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        VerticalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.30, rotate_limit=89, prob=.75),
        ImageOnly(RandomBrightness()),
