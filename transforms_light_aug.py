...
class RandomBrightness:
    def __init__(self, limit=0.05, prob=0.5):
        self.limit = limit
        self.prob = prob
...
class RandomContrast:
    def __init__(self, limit=.05, prob=.5):
        self.limit = limit
        self.prob = prob
...
def augment_a_little(x, mask=None, prob=.5):
    return DualCompose([
        ...
        VerticalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.10, rotate_limit=45, prob=.75)
    ])(x, mask)
...
def augment_color(x, mask=None, prob=.5):
    return DualCompose([
        HorizontalFlip(prob=.5),
        VerticalFlip(prob=.5),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.1, rotate_limit=30, prob=.75),
