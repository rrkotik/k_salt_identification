...
from transforms import augment_shift_flip_scale, augment_shift_flip_scale1, augment_shift_flip_scale2, augment_shift_flip, \
    augment_shift_flip_scale, augment_shift_flip_rotate, augment_shift, augment_shift_flip_scale3, augment_shift_flip_color
...
class PytorchTrain:
    ...
    def __init__(self, fold, config, metrics):
        ...
        if self.config.optimizer == 'adam':
            self.optimizer = optim.Adam(learnable_parameters, lr=config.lr)
        else:
            self.optimizer = optim.RMSprop(learnable_parameters, lr=config.lr)
    ...
    def fit(self, train_loader, val_loader):
        ...
        for epoch in range(self.config.nb_epoch):
            ...
            loss = -float(val_metrics[TARGET_METRIC])
            ...
            if epoch >= self.config.warmup_epoch and epoch < self.config.cycle_start_epoch:
...
def train_config(config_path):
    ...
    image_folder_name = 'train/images'
